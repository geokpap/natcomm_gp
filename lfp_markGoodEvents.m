%LFP_MARKGOODEVENTS
% Script for use by lfp_parseEyeTraces, NOT coded as a function in
...
for idx = 1:length(rawevents)
    ...
    if (pBlinkIdx < length(blinkends)) && ...
        ...
        if nBlinkIdx == length(blinkstarts) && blinkstarts(end) > blinkends(end)
            % do nothing
    ...
    % end and NRecTimeout after the preceding Record On:
    if (pBlinkIdx > 0) && (rawevents(idx) < blinkends(pBlinkIdx) + NBlinkTimeout)
        eventgood(idx) = false;
    end
    ...
    % and NRecTimeout before the next Rec Off:

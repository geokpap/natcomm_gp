%LFP_MARKGOODEVENTS2
% Script for use by lfp_parseEyeTraces2, NOT coded as a function in
...
for idx = 1:length(rawevents)
    ...
    if (pFailedIdx < length(failedBlinks)) && ...
            (rawevents(idx) > failedBlinks(pFailedIdx + 1))
        if nFailedIdx > length(failedBlinks)
            % also do nothing
        else
    ...
    % event times must be at least NBlinkTimeout after the preceding blink
    % end or ambiguous sacc/blink, and NRecTimeout after the preceding
    % Record On:
    if (pBlinkIdx > 0) && ...
            (rawevents(idx) < blinkends(pBlinkIdx) + NBlinkTimeout)
        eventgood(idx) = false;
    end
    if (pFailedIdx > 0) && ...
            (rawevents(idx) < failedBlinks(pFailedIdx) + NBlinkTimeout)
        eventgood(idx) = false;
    end
    if (rawevents(idx) < lfp_RecSegments(recidx, 1) + NRecTimeout)
        eventgood(idx) = false;
    end
    % ... and also at least NBlinkTimeout before the next blink start
    %  or ambiguous sacc/blink, and NRecTimeout before the next Rec Off:
    if (nBlinkIdx <= length(blinkstarts)) && ...
            (rawevents(idx) > blinkstarts(nBlinkIdx) - NBlinkTimeout)
        eventgood(idx) = false;
    end
    if (nFailedIdx <= length(failedBlinks)) && ...
            (rawevents(idx) > failedBlinks(nFailedIdx) - NBlinkTimeout)
        eventgood(idx) = false;
    end
    if (rawevents(idx) > lfp_RecSegments(recidx, 2) - NRecTimeout)
        eventgood(idx) = false;
    end
end

function [ts, saccwave, mergethresh, revtrimthresh] = ...
    lfp_parseEyeTraces8(eyeX, eyeY, velo, accel, threshes, vpeakthresh, ...
    blinkstarts, blinkends, evtbounds, figflag)
%INPUTS
...
%   structure are:
%   blinkTO - time out around each blink during which lfp_parseEyeTraces8
...
%       saccades
%   maxfixdiam - default value for maximum "normal" fixation diameter, in
%       case it cannot be determined by analyzing data.
%   minfixdur - duration threshold below which "fixations" are not marked
%   minsaccsize - the smallest distance allowed to be covered by a "good"
%       saccade, in whatever units <eyeX> and <eyeY> are in.
...
%   blinkv1.
% evtbounds: optional; defaults to [122 123].  Only the portions of each
%   trial that are between the trial's <evtbounds> events get analyzed.
% figflag: optional; default is <true> (plots histogram figures).
%OUTPUTS
% <ts> is a cell vector of timestamp column vectors, with one column for
% each of: 
% 1.	  Start Blink
% 2.	  End Blink
% 3.	  Start Simple Saccade
% 4.	  End Simple Saccade
% 5.	  Start Complex Saccade (i.e. not fittable with single Gaussian)
% 6.	  End Complex Saccade
% 7.	  Post-Blink Fixation Starts, <blinkfixstarts>
% 8.	  Post-Blink Fixation Ends, <blinkfixends>
% 9.	  Other Fixation Starts
% 10.	  Other Fixation Ends
% 11.	  Bad Fixation Starts (when adjustment fails)
% 12.	  Bad Fixation Ends (when adjustment fails)
% 13.	  Start Merged Saccade Series (series of disjoint saccades that
%           should still be merged)
% 14.	  End Merged Saccade Series
% 15.	  Fit Starts (returned from 'fitsaccade')
% 16.	  Fit Ends (returned from 'fitsaccade')
% 17.	  Fixations for Adjustment (marked at midpoint of unadjusted
%           fixation)
% 18.	  Old Fixation Starts (before adjusting fixations)
% 19.	  Old Fixation Ends (before adjusting fixations)
% 20.	  Blink-Terminated Sacc Starts
% 21.	  Blink-Terminated Sacc Ends
...
%	"lfp_parseEyeTraces8 & lfp_findBlinks.doc".  Any changes made to the
%	code should entail corresponding changes to  "lfp_parseEyeTraces8.doc".
% 2) Use <evtbounds> to override Theresa events 122 and 123.
...
% Bunches-o-functions that all get called twice:

% The variables otherfixstarts, otherfixends, oldotherfixstarts,
% oldotherfixends, badotherfixstarts, badotherfixends, midotherfix are
% returned from 'adjustfixations', and the variables oldfixstarts,
% badfixstarts, oldfixends, badfixends, midfix are derived from them
% (see "Adjust the fixations").

% 'fitsaccade' does the actual saccade-by-saccade waveform fitting, but 
% fittedwave, saccwaverange, saccstartpt, saccendpt are all returned from
% 'multigauss' based on that fitting.

% There is also the 'adjustsaccend' function.

% 'fitsaccade' is called from two different points, one for single saccades
% and one for merged runs.
% <startfit>, <endfit>: sample numbers of time interval fitted, returned
% from 'fitsaccade'.

% There are two categories of sets of fixations created by the two calls to
% 'adjustfixations', the first of which derive from "blink fixations" and
% the second from "other fixations":
%   1. blinkfix, oldblinkfix, badblinkfix, midblinkfix
%   2. otherfix, oldotherfix, badotherfix, midotherfix

%$Rev: 424 $
%$Date: 2023-10-05 16:36:36 -0400 (Thu, 05 Oct 2023) $
%$Author: dgibson $

global lfp_Samples lfp_SamplePeriod lfp_SelectedTrials lfp_Events ...
    lfp_OrigTrialNums lfp_SessionNames lfp_DataDir lfp_RecSegments %#ok<*GVMIS> 

lfp_declareGlobals;
lfp_log('lfp_parseEyeTraces8 finding saccades');
...
switch vpeakthresh
    ...
    otherwise
        error('lfp_parseEyeTraces8:badvpeakthresh', ...
...
% Find the ones that are in <evtbounds> interval for histogram analysis:
...
for trial = 1:length(lfp_SelectedTrials)
    ...
    eix122 = find(trialevts(:,2)==evtbounds(1)) ...
        + lfp_TrialIndex(trial,1) - 1;
    eix123 = find(trialevts(:,2)==evtbounds(2)) ...
        + lfp_TrialIndex(trial,1) - 1;
    if ~isempty(eix122) && ~isempty(eix123)
        inbounds( ...
            (vpeaks >= lfp_time2index(lfp_Events(eix122,1))) & ...
            (vpeaks <= lfp_time2index(lfp_Events(eix123,1))) ) = true;
    else
        warning('lfp_parseEyeTraces8:evtbounds', ...
            'An evtbounds event is missing from trial %d', trial );
    end
end      
boundedbigsaccs = vpeaks(inbounds);

% Find runs of close-interval saccades and compute intra-run v-peak ratios
...
if length(runstartidx) < maxscale * 100
    logmsg = 'Not enough data inside evtbounds for reliable v-peak ratios histo, trying whole trials';
    lfp_log(['lfp_parseEyeTraces8: ' logmsg]);
    warning('lfp_parseEyeTraces8:dubious3', '%s', logmsg);
    saccs2plot = unique(vpeaks);
    saccsdif = diff(saccs2plot);
    is_short = saccsdif < Nminsaccint;
    % Find first v peak in each run in whole trial:
    if isempty(is_short)
        runstartidx = [];
    else
        runstartidx = find([ is_short(1)
    ...
    if length(runstartidx) < maxscale * 100
        ...
        lfp_log(['lfp_parseEyeTraces8: ' logmsg]);
        warning('lfp_parseEyeTraces8:dubious4', '%s', logmsg);
    end
end
...
% <mergethresh> is used in this top-level function ('lfp_parseEyeTraces8')
% to determine whether temporally overlapping saccades should be merged
% into one long "complex" saccade, or whether the "extra" saccades should
% be considered corrective and removed.  It is determined by analyzing a
% histogram of either first-to-second peak amplitude ratios, or
% first-to-third peak amplitude ratios.
% Default value when histo fails:
mergethresh = 0.6;
...
if isempty(runstartidx)
    ...
    lfp_log(['lfp_parseEyeTraces8: ' logmsg]);
    warning('lfp_parseEyeTraces8:ratiohisto1', '%s', logmsg);
else
    ...
    rawhisto = reshape(histc(ratios, binedges), [], 1); %#ok<*HISTC> 
    if ~isempty(ratios2)
    ...
    [~, maxbin] = max(rawhisto);
    [~, onebin] = min(abs(binctrs-1));
    if maxbin > onebin
        logmsg = 'V peak ratio histo has max above 1';
        lfp_log(['lfp_parseEyeTraces8: ' logmsg]);
        warning('lfp_parseEyeTraces8:ratiohisto2', '%s', logmsg);
    end
    [~, pt6bin] = min(abs(binctrs-0.6));
    [~, lowmaxbin] = max(rawhisto(1:pt6bin));
    if lowmaxbin == 1
        logmsg = 'V peak ratio histo has low max at first bin';
        lfp_log(['lfp_parseEyeTraces8: ' logmsg]);
        warning('lfp_parseEyeTraces8:ratiohisto3', '%s', logmsg);
    end
    ...
    while (nummin>3 && k<maxiters)
        smoothfracstrs{k,1} = sprintf('%.4f', smoothfrac); %#ok<*AGROW> 
    ...
    if k >= maxiters
        warning('lfp_parseEyeTraces8:maxiters', ...
        ...
        if nummin > 0
            ...
            warning('lfp_parseEyeTraces8:nomergethresh', ...
                'Could not find value for mergethresh in ratio histogram');
        end
    end

    % plot the analyzed histograms
    if figflag
        hF = figure;
        hA = axes;
        ...
        legend(['raw'; smoothfracstrs; 'mergethresh'; 'revtrimthresh']);
...
if ismember('vpeakratiomergethresh', fieldnames(threshes)) && ...
    ...
    mergethresh = threshes.vpeakratiomergethresh;
    warning('lfp_parseEyeTraces8:override1', '%s', msg);
    lfp_log(msg);
end
if ismember('vpeakratiorevtrimthresh', fieldnames(threshes)) && ...
    ...
    revtrimthresh = threshes.vpeakratiorevtrimthresh;
    warning('lfp_parseEyeTraces8:override2', '%s', msg);
    lfp_log(msg);
end
lfp_log(sprintf( ...
    'lfp_parseEyeTraces8 starting saccade grouping, mergethresh=%d revtrimthresh=%d minsaccint=%d', ...
    mergethresh, revtrimthresh, minsaccint ));

% Find the vpeaks that span >= minsaccsize in space.  Since vpeaks are
...
isgoodpeak = peakspans >= threshes.minsaccsize;
...
%
% <run1idx2> indexes <runstartidx>, <runendidx>.  Its members are isolated
% pairs, which are handled the same as runs.  They should all end up in
...
if ~isempty(runstartidx) && any(ismember(runstartidx(runlength==1), isolatedsaccidx))
    error('lfp_parseEyeTraces8:bug2', ...
        'Something impossible happened.  Please despair.');
end
% <mergeidx> contains pairs of indices into <bigsaccs> for runs of saccades
% to merge. col 1: starting bigsaccs idx; col 2: ending bigsaccs idx.
mergeidx = [];  
...
for runnum = 1:length(runstartidx)
    ...
    if median(lfp_Samples{velo}(rhinowindow)) > threshes.blinkv1
        ...
        if relmergestart > relmergeend
            warning('lfp_parseEyeTraces8:bogusrun', ...
...
fprintf('Processing single saccades...\n');
rmses = cell(1,6); 
for saccidx2 = 1:length(singlesaccidx)
    if ~mod(saccidx2, 1000)
        fprintf('%d/%d saccades\n', saccidx2, length(singlesaccidx));
    end
    saccidx = singlesaccidx(saccidx2);
    % <term> specifies the type of event that terminates the saccade.
    % See "<category>:" in 'fitsaccade' header comments saccade categories.
    [c, ~, gof, category, startfit, endfit, term] = fitsaccade( ...
        velo, accel, bigsaccs, saccidx, threshes, blinkstarts);
    if isempty(c)
        warning('lfp_parseEyeTraces8:nofit1', ...
    ...
    % <startfit> and <endfit> are returned from 'fitsaccade':
    fitstarts(saccidx(1)) = startfit;
    fitends(saccidx(1)) = endfit;
end

% Then the merged runs.
fprintf('Processing %d merged runs...\n', size(mergeidx,1));
for mergeidx2 = 1:size(mergeidx,1)
    ...
    if isempty(coeffs)
        warning('lfp_parseEyeTraces8:nofit2', ...
            'Fitting for saccade at t=%s was skipped.', ...
            mat2str(lfp_index2time(bigsaccs(saccidx))) );
        continue
    end
...
fprintf('Searching for nested saccades...\n');
events = [
    saccstarts, ones(size(saccstarts))
...
for evtnum = 1:size(events,1)
    ...
    if events(evtnum, 2) < 0
        ...
        if nestlevel == 0 && eventtype ~= -events(evtnum, 2)
            ...
            if eventtype == 3
                warning('lfp_parseEyeTraces8:ummm', ...
...
% variables.
fprintf('Constructing fixations...\n');
...
fprintf('Constructing functional saccade boundaries...\n');
...
events = [
    blinkstarts2, ones(size(blinkstarts))
...
if any(isrepeat)
    warning('lfp_parseEyeTraces8:repeatevts', ...
    ...
    for id = reshape(repIDs, 1, [])
        ...
        if sum(isrepstart) ~= sum(isrepend)
            warning('lfp_parseEyeTraces8:unbalrep', ...
...
fprintf('Running FSM to complete parsing...\n');
inblink = false;
inrecTO = true;
insacc = false;
infix = false;
...
for k = 1:size(events,1)
        ...
        if any([inblink inrecTO insacc])
            ...
            if wasblink
                ...
                if blinkfixstarts(fixnum) == 0 ...
                    ...
                    warning('lfp_parseEyeTraces8:missedblinkfix', ...
                        '%s', msg );
                    lfp_log(msg);
                ...
                if otherfixstarts(fixnum) == 0 ...
                    ...
                    warning('lfp_parseEyeTraces8:missedotherfix', ...
                        '%s', msg );
                    lfp_log(msg);
    ...
    elseif abs(events(k,2)) == 4
        ...
        insacc = false;
        infix = false;
        % recTO ends should also start a new fixation. 
        if events(k,2) == -2
            infix = true;
            if events(k,1) < numel(lfp_Samples{velo})
                otherfixstarts(fixnum) = events(k,1) + 1;
...
% Find largest "normal" size for fixations if possible; if not, default to
% threshes.maxfixdiam.
if length(otherfixdiams) < 50
    maxdiam = threshes.maxfixdiam;
    lfp_log(sprintf( ...
...
if endfit == lastsample2fit
    ...
    if lastminidx == bigsaccs(saccidx(1))
        ...
        %warning('lfp_parseEyeTraces8:endfit', '%s', msg); %just log
        lfp_log(msg);
...
switch numpeaks
    ...
    case num2cell(2:(length(funcnames)-1))
        category = 3;
...
while true
    ...
    if k > NmaxT
        if ~isempty(highminvpt)
            ...
            %warning('lfp_parseEyeTraces8:badsaccend', ...
                %'%s', msg); %just log
            lfp_log(msg);
            break
        end

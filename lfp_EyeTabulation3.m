function [fixations, saccades] = lfp_EyeTabulation3(xnum, ynum, vnum, ...
...
% left, y<0 = down.
% <fixEvtIDs>, <saccEvtIDs>: 2-column arrays with starting events in column
% 1 and ending events in column 2.  Typically, these are matching pairs of
% events that qualify as delimiters for fixations and saccades
% respectively, but they don't have to come in pairs, can be in any order,
% and can be padded with NaNs if necessary to make them rectangular.
%OUTPUTS
...
% from per sample to per second, divide by lfp_SamplePeriod).  
%OPTIONS
...
%   'preset', files - unconditionally creates output files, using the
%       2-element cell array <files> as output filenames; note that if
%       <files> does not contain absolute pathnames, then the output files
%       will be written to the current working directory.  <files>{1} is
%       the fixations table filename <fixOutputFileName>, and <files>{2} is
%       the saccades table filename <saccOutputFileName>.
...
%NOTES
% Started with a copy of lfp_EyeTabulation of $Rev: 424 $Date: 2008-12-14
% 16:07:41 -0500, merged in selected changes from lfp_EyeTabulation2
% without the major rewrites, to produce a more up-to-date but user-neutral
% version.  Removed GUI features.
%   Saccades are identified as starting with any of <saccEvtIDs(:,1)> and
% ending with any of <saccEvtIDs(:,2)>.  Fixations start with any of
% <fixEvtIDs(:,1)> and ending with any of <fixEvtIDs(:,2)>.
%   Saccades and fixations are included only if they are contained
% completely within the boundaries of the trial, the starting event is
% after lfp_TrialIndex(trial,1) and the ending event is before
% lfp_TrialIndex(trial,2).
%   The typical pipeline is:
% 1. lfp_findBlinks
% 2. create eye events: lfp_createEvents(@lfp_parseEyeTraces8, ...)
% 3. remove "post-blink junk": lfp_removeEyeJunk
% 4. lfp_saccadeCleanup
% 5. lfp_EyeTabulation3

%$Rev: 424 $
%$Date: 2023-10-05 16:36:36 -0400 (Thu, 05 Oct 2023) $
%$Author: dgibson $

global lfp_Events lfp_TrialIndex lfp_Samples %#ok<GVMIS> 
lfp_declareGlobals;
lfp_getEvtIDs;
...
for trialidx = 1:length(trials)
        ...
        if extendflag
            ...
            for evtidx = lfp_TrialIndex(trial,2) + 1 : size(lfp_Events, 1)
                if ismember(lfp_Events(evtidx, 2), fixEvtIDs(:,2))
                    ...
                    foundEnd = true;
                    fixends(end+1) = size(trialevents, 1) + 1; %#ok<AGROW> 
                    trialevents(end+1, :) = lfp_Events(evtidx, :); %#ok<AGROW> 
    ...
    if do_saccades
        ...
        if ~isempty(extraSacc)
            ...
            rows2delete = ismember(pairs(:,2), preceding);
            pairs(rows2delete,:) = [];
        end
        if isempty(extraFix)
            noFinalFix = true;
        ...
        if noFinalFix
            ...
            undefSacc(end+1) = length(saccstarts); %#ok<AGROW> 
    ...
    if do_saccades
        ...
        % direction
        s = warning('off', 'MATLAB:divideByZero'); %#ok<RMWRN> 
...
fprintf(...
    'Total number of trials skipped: %d\n', numSkipped );

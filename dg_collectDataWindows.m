function [windows, winsamples] = dg_collectDataWindows(data, ...
    offsets, window, noclip, nodataOKflag, varargin)
%[windows, winsamples] = dg_collectDataWindows(channel, trials, ...
%    evtbounds, offsets, window)
% Same as lfp_collectDataWindows, except it accepts a literal data array
...
if ~isa(trials, 'double') || ~all(fix(trials) == trials)
    error('dg_collectDataWindows:badTrials2', ...
        '<trials> must be an integer vector.');
end

if ~iscell(evtbounds)
    error('dg_collectDataWindows:badevtbounds', ...
        '<evtbounds> must be a 1 X 2 cell array.');
elseif ~isequal(size(evtbounds), [1 2])
    error('dg_collectDataWindows:badevtbounds2', ...
        '<evtbounds> must be a 1 X 2 cell array.');
end

if ~isnumeric(offsets)
    error('dg_collectDataWindows:offsets', ...
        '<offsets> must be a 1 X 2 numeric array.');
elseif ~isequal(size(evtbounds), [1 2])
    error('dg_collectDataWindows:offsets2', ...
        '<offsets> must be a 1 X 2 numeric array.');
end

if ~isnumeric(window)
    error('dg_collectDataWindows:window', ...
        '<window> must be a scalar number.');
elseif ~isequal(size(window), [1 1])
    error('dg_collectDataWindows:window2', ...
        '<window> must be a scalar number.');
end

if ~isnumeric(margintime)
    error('dg_collectDataWindows:margintime', ...
        '<margintime> must be a scalar number.');
elseif ~isequal(size(margintime), [1 1])
    error('dg_collectDataWindows:margintime2', ...
        '<margintime> must be a scalar number.');
end
...
elseif ~isempty(noclip) && ( .../
        ...
        || ~isnumeric(noclip) )
    error('dg_collectDataWindows:noclip', ...
...
for trialidx = 1:length(trials)
    ...
    if isempty(startevtidx)
        errid = 'dg_collectDataWindows:evtbounds1';
        errmsg = sprintf(...
    ...
    if isempty(endevtidx)
        errid = 'dg_collectDataWindows:evtbounds2';
        errmsg = sprintf(...
    ...
    if startsample < lfp_TrialRec(trial,1)
        errid = 'dg_collectDataWindows:startsample';
        errmsg = sprintf(...
    ...
    elseif endsample > lfp_TrialRec(trial,2)
        errid = 'dg_collectDataWindows:endsample';
        errmsg = sprintf(...
...
if ~isempty(noclip)
    ...
    msg = sprintf( ...
        'dg_collectDataWindows eliminated %d clipped windows, of %d', ...

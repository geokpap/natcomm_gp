...
%WARNING: THIS FUNCTION HAS REACHED THE "LAVA FLOW" STAGE WHERE IT IS SO
%COMPLICATED AND DISORGANIZED THAT NOTHING CAN BE MODIFIED OR EVEN
%UNDERSTOOD.  ALL FUTURE ENHANCEMENTS SHOULD BE IMPLEMENTED AS NEW
%FUNCTIONS.  - DG 09-May-2012 17:54:54
%PS: THAT GOES FOR LFP_MULTICHANNEL_PLOT EVEN MORE SO!
...
%  lfp_SetupType = 'monkey', 'theresa', 'rodent', 'georgios').  Behavior is
%  undefined when combined with 'ovr' or 'avg' (for lfp_SetupType =
%  'rodent', specifying multiple trials to plot automatically overlays).
...
%lfp_disp(..., 'nantolerant')
%  Modifies computation of 'avg' values and 'err2' SDs so that at each time
%  point any trials containing NaN are simply skipped, and ultimately the
%  2*SEM saved in avgwave(:,filenumidx,2) is based on the number of non-NaN
%  values at each time point.  This option is a late addition, and will
%  probably not work with whatever (combination of) options motivated the
%  use of the running-total method for computing mean and SD, but it has
%  NOT BEEN TESTED with anything other than 'avg', 'err2'; but it should
%  work with the basic batching mechanism used by 'avg', 'err2'.
%lfp_disp(..., 'noplot')
...
%  lfp_ClickedTrials.  (Appears to be bugged under as yet unknown
%  conditions. - DG 2019-11-29)
...
%  then xcov(x, y, 'coeff') is called, if not then xcov(x, y, 'unbiased').
...
%$Rev: 426 $
%$Date: 2023-11-19 15:51:09 -0500 (Sun, 19 Nov 2023) $
%$Author: dgibson $
...
if multitrigflag && ovrflag
    warning('lfp_disp:badcombo', ...
        '''ovr'' may not work with ''multitrig'', in which case try lfp_trigger.');
end
...
elseif ovrflag & avgflag
    option = 'ovrchan';
    warning('lfp_disp:ovrchan', ...
        'The combination of ''ovr'' with ''avg'' sometimes produces an incorrect plot; if it looks wrong, it probably is.');
elseif avgflag
    option = 'errorcurves';
else
    option = '';
        ...
        switch lfp_SetupType
            case 'monkey'
                ...
                if ~startedplotting
                    ...
                    plot_int = 1; % initialize # of figures
            ...
            case 'georgios'
                ...
                subsampinterval = round(0.005 / lfp_SamplePeriod);
                subsamples = startsamp:subsampinterval:endsamp;
                rel_trialtime = (subsamples - refpoint) * lfp_SamplePeriod;
                blinklevel = 0.8 * min(lfp_Samples{filenums(1)}(:));
                xdata = lfp_Samples{filenums(1)}(subsamples);
                ydata = lfp_Samples{filenums(2)}(subsamples);
                pts2delete = (xdata < blinklevel | ydata < blinklevel);
                xdata(pts2delete) = [];
                ydata(pts2delete) = [];
                rel_trialtime(pts2delete) = [];
                scatter(hA, xdata, ydata, 10, ...
                    rel_trialtime, 'filled');
                hCB = colorbar('peer', hA);
                set(get(hCB,'YLabel'),'String','Time (s)');
                set(hA, 'XGrid', 'on');
                set(hA, 'YGrid', 'on');
            otherwise
                % formerly limited to 'simple' 7/27/12 -DG
                if isempty(hF)
                    hF = figure;
                    hA = axes('Parent', hF);
                    set(hA, 'NextPlot', 'add');
...
if avgflag || ovrflag
    ...
    for filenumidx = 1:length(filenums)
        ...
        if ~(avgxcovflag || ovrflag) || ~isempty(evts2avg)
            ...
            while numtrialsleft > 0
                ...
                % subtotwave(:,batchnum) is a column vector of samples:
                if nantolflag
                    pts2use = ~isnan(samplematrix2);
                    % <numtrialswave(:,batchnum)> is a column vector of
                    % samples:
                    numtrialswave(:,batchnum) = sum(pts2use, 1)'; 
                    subtotwave(:,batchnum) = nansum(samplematrix2, 1)';
                else
            ...
            clear samplematrix2;
            % <avgwave> is {samples X filenums}
            if nantolflag
                avgwave(:,filenumidx) = sum(subtotwave, 2) ./ ...
                    sum(numtrialswave, 2);
            else
                % sum of all batches divided by number of trials:
            ...
            if errflag || err2flag || err4flag
                ...
                while numtrialsleft > 0
                    ...
                    % <subtotdiffsq(:,batchnum)> is a column vector of
                    % samples.
                    % <diffmatrix(trials, samples)>.
                    % <subtotdiffsq(samples, batchnum).
                    if nantolflag
                        subtotdiffsq(:,batchnum) = nansum(diffmatrix.^2,1)';
                    else
                ...
                clear diffmatrix;
                if nantolflag
                    stdwave = sqrt(sum(subtotdiffsq,2) ./ ...
                        (sum(numtrialswave, 2) - 1));
                else
    ...
    % plot the avg waves
    if figflag
        plot_int = 999+figbase;
    else
        if plotflag
            plothandle = figure('Visible', display_str);
        else
            plothandle = [];
        end
    end
    ...
    if xcovflag
        ...
        if avgxcovflag || ovrflag
            for plotnum = 1:numplots
                ...
                for trialidx = 1:size(trialinfo, 1)
                    xc(trialidx, :) = xcov( samplematrix(trialidx, :, 2*plotnum-1), ...
                        samplematrix(trial_vector(trialidx), :, 2*plotnum), xcov_opt );
                end
                if avgxcovflag
            ...
            for plotnum = 1:numplots
                xcovs(:, plotnum) = xcov( ...
                    avgwave(:, 2*plotnum-1), ...
                    avgwave(:, 2*plotnum), xcov_opt );
            end
        end
        reftime2 = (size(avgwave,1) - 1) * lfp_SamplePeriod;
        [hF, data] = lfp_multichannel_plot(plothandle, titlestr, ...
            plotnames, xcovs, [], ...
            reftime2, axinfo, option);
    else % ~xcovflag

%LFP_MARKGOODEVENTS4
% Script for use by lfp_parseEyeTraces4, NOT coded as a function in
...
for idx = 1:length(rawevents)
    ...
    if ~isempty(pBlinkIdx) && ~isempty(nBlinkIdx) && ...
            (pBlinkIdx < length(blinkends)) && ...
    ...
    % end and NRecTO after the preceding Record On:
    if (pBlinkIdx > 0) && (rawevents(idx) < blinkends(pBlinkIdx) + NBlinkTO)
        eventgood(idx) = false;
    end
    ...
    % and NRecTO before the next Rec Off:
    if (nBlinkIdx <= length(blinkstarts)) && ...

...
global analyzeLog_bill_output_str;

analyzeLog_bill_output_str = [];
analyzeLog_bill_cluster_count = 0;
...
while ~isequal(line, -1)
    ...
    elseif length(line) > 46 && ...
        ...
        analyzeLog_bill_cluster_count = ...
            analyzeLog_bill_cluster_count + nclusts;
...
fclose(logfid);
analyzeLog_bill_output_str = [
    sprintf('logfile:\t%s\tthresh:\t%d\tclusters:\t%d\n\n', ...
    filepath, thresh, analyzeLog_bill_cluster_count  ) ...
    sprintf('sessiondir\tcluster\tmaxdsc\tlfp#\tsametrode\t# trodes\tXLim\tFreqLim\tAlign\n') ...
    analyzeLog_bill_output_str ];
fprintf(outfid, '%s', analyzeLog_bill_output_str);
fclose(outfid);
...
if ~isempty(suprathresh)
    ...
    analyzeLog_bill_output_str = [ analyzeLog_bill_output_str ...
        sprintf('%s\t%s\t%f\t%i\t%s\t%d\t%s\t%s\t%s\n', ...
        sessiondir, spike{idx}, maxdsc, lfptrodenum, mat2str(sametrode), ...
        length(suprathresh), mat2str(xlim'), mat2str(flim'), ...
        dg_thing2str(align) ) ];
end

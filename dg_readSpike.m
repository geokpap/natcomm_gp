function [TS, Samples, Hdr, nlxjunk] = dg_readSpike(filename, varargin)
...
%       named as in the Nlx2MatSpike_411 header comments (or see code
%       here).
% OPTIONS
...
if ispc
    ...
    if headeronly
        Hdr = Nlx2MatSpike_411(file2read, [0, 0, 0, 0, 0], 1, 1);
    else
        if nargout > 2
            if nargout > 3
                [TS, nlxjunk.ScNumbers, nlxjunk.CellNumbers, ...
                    nlxjunk.Params, Samples, Hdr] = Nlx2MatSpike_411( ...
                    file2read, [1 1 1 1 1], readheader, modenum, modearg);
            else
                [TS, Samples, Hdr] = Nlx2MatSpike_411(file2read, ...
                    selectary, readheader, modenum, modearg);
            end
        elseif nargout == 2
            [TS, Samples] = Nlx2MatSpike_411(file2read, selectary, ...
                readheader, modenum, modearg);
        else
            % nargout must be 1
            TS = Nlx2MatSpike_411(file2read, selectary, ...
                readheader, modenum, modearg);
        end
    end
elseif ismac || isunix

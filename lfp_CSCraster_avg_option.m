function [hF, hI, hCB, data] = lfp_CSCraster(trials, filenums, window, ...
    varargin)
% I started adding an 'avg' option to this and then realized that it
% wouldn't help with what I was doing.  See "Simon Data Analysis 2.doc" for
% details.
...
%   <trials> of <filenums> may be a non-scalar array, unless the 'avg'
%   option is specified.  The one that has multiple elements will be used
%   for the Y axis.  <filenums> may contain elements whose value is NaN,
%   and a row of all NaNs will be displayed at those positions; this is
%   meant to facilitate visually segregating channels into groups.
%OUTPUTS
% hF: figure handle
...
%   format.
%OPTIONS
% 'avg' - produces the same sort of display as when a single trial is shown
%   for multiple filenums, but instead of plotting single trials, each
%   trace plots the average waveform over all enabled trials.
...
if ~avgflag && numel(trials) > 1 && numel(filenums) > 1
    ...
    if ~avgflag && numel(trials) > 1
        ...
        for filenumidx = 1:length(filenums)
            if isnan(filenums(filenumidx))
                ...
                if avgflag
                    % average across trials before assigning to <data>
                    disp('foob');
                else

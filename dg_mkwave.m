...
%   form - can be 'sin', 'sqr', 'tri', 'table', 'pink', 'func'.  The value
%       'table' indicates that the waveform is specified by interpolating
%       values from a table; 'pink' specifies pink noise; 'func' specifies
%       applying a function handle to the already-existing <subwaves> to
%       yield the new subwave (see 'param').
...
%       exp(-2*pi*fcutoff/Fs).  For 'func' it is a function handle to a
%       function of one argument, namely <subwaves>, that returns a vector
%       of sample values of length size(subwaves, 1).  Note that all the
%       following fields of <wavespec> have no effect when using 'func'.
...
%$Rev: 303 $
%$Date: 2023-01-05 20:34:05 -0500 (Thu, 05 Jan 2023) $
%$Author: dgibson $
...
for cidx = 1:length(wavespec)
    ...
    switch wavespec(cidx).form
        case 'func'
            subwaves(:, cidx) = feval(wavespec(cidx).param, subwaves);
        case 'sin'
            subwaves(:, cidx) = amp .* ...

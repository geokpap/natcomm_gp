...
% Check and display the actual number of elements in List2
disp(['Number of entries in List2: ', num2str(numel(List2))]);

% Assuming List2 is the structure with fields 'name' and 'folder'
numEntries = numel(List2);  % Get the actual number of entries
malakas = strings(numEntries, 4);  % Initialize using the built-in strings function

% Loop through each entry in List2
for i = 1:numEntries
    % Safely accessing fields within bounds
    malakas(i, 1) = List2(i).name;
    
  % Column 2: Extract and format the date from the folder field
dateMatch = regexp(List2(i).folder, '\\(?:ApAvApAp|ApAv_em|ApAv)\\(\d{4}-\d{1,2}-\d{1,2})', 'tokens');
% Loop through List2 elements
for i = 1:length(List2)
    % Using regexp to extract the date string
    dateMatch = regexp(List2(i).folder, '\\(?:ApAvApAp|ApAv_em|ApAv)\\(\d{4}-\d{1,2}-\d{1,2})', 'tokens');

    if ~isempty(dateMatch) && ~isempty(dateMatch{1})
        % Using datetime to parse and format the date string directly
        dateString = dateMatch{1}{1};  % Extracting the date string from the first match's first token
        try
            dateValue = datetime(dateString, 'InputFormat', 'yyyy-MM-dd', 'Format', 'M/d/yyyy');
            malakas{i, 2} = char(dateValue);  % Ensure output is in string format
        catch ME
            disp(['Error converting date for index ', num2str(i), ': ', ME.message]);
            malakas{i, 2} = '';  % Set to empty string or some default value on failure
        end
    else
        disp(['No valid date found for index ', num2str(i)]);
        malakas{i, 2} = '';  % Set to empty string or some default value
    end
    ...
    if ~isempty(personMatch)
        malakas(i, 3) = personMatch{1}{1};
    end

    % Extract animal's name
    animalMatch = regexp(List2(i).folder, '\\Mnks\\(.*?)\\', 'tokens');
    if ~isempty(animalMatch)
        malakas(i, 4) = animalMatch{1}{1};
    end
end
end

% Extract the first column for suffix checking and the second column for dates
suffixes = malakas(:, 1);
dates = malakas(:, 2);

% Initialize an empty cell array for the replicated dates
replicatedDates = {};

for i = 1:length(suffixes)
    currentSuffix = suffixes{i};
    currentDate = dates{i};
    
    % Determine the number of replications based on the suffix (in1, in2, in3)
    repeatTimes = 1; % Default to 1
    if contains(currentSuffix, 'in3')
        repeatTimes = 3;
    elseif contains(currentSuffix, 'in2')
        repeatTimes = 2;
    elseif contains(currentSuffix, 'in4')
        repeatTimes = 4;    
    end
    
    % Replicate the current date the determined number of times
    for j = 1:repeatTimes
        replicatedDates{end+1, 1} = currentDate; % Append to replicatedDates
    end
end

dates=replicatedDates;

% Extract the first column from malakas
firstColumn = malakas(:, 1);

% Initialize an empty cell array for fileNames and an empty array for the counts
fileNames = {};
counts = [];

for i = 1:length(firstColumn)
    currentString = firstColumn{i};
    
    % Remove the .nex suffix
    currentString = erase(currentString, '.nex');
    
    % Determine the repetition based on in2 or in3 within the string
    repeatTimes = 1; % Default to 1
    if contains(currentString, 'in3')
        repeatTimes = 3;
    elseif contains(currentString, 'in2')
        repeatTimes = 2;
    elseif contains(currentString, 'in4')
        repeatTimes = 4;
    end
    
    % Repeat the string the determined number of times
    for j = 1:repeatTimes
        fileNames{end+1, 1} = currentString; % Append to fileNames
        counts(end+1, 1) = j; % Append the repetition count for this occurrence
    end
end

counts1=num2cell(num2str(counts));

% Extract the fourth column from malakas, which contains the monkey names

fourthColumn = malakas(:, 4);
% Initialize an empty cell array for processedNames
monkey_name = {};

for i = 1:length(fourthColumn)
    currentName = fourthColumn{i};
    
    % Remove the .nex suffix from the current name
    currentName = erase(currentName, '.nex');
    
    % Determine the repetition based on substrings like in2, in3, or in4 within the name
    % But now look for these substrings in the firstColumn instead of the currentName
    repeatTimes = 1; % Default to 1
    if contains(firstColumn{i}, 'in3')
        repeatTimes = 3;
    elseif contains(firstColumn{i}, 'in2')
        repeatTimes = 2;
    elseif contains(firstColumn{i}, 'in4')
        repeatTimes = 4;
    end

    % Create a temporary cell array with the name repeated the determined number of times
    tempArray = repmat({currentName}, repeatTimes, 1);

    % Concatenate the temporary array with the main monkey_name array
    monkey_name = [monkey_name; tempArray];
end
...
allInfoTogether=[fileNames,dates,brik];
% allInfoTogether = cellstr(allInfoTogether);

allMySpikes_Pavlovian = {}; % Initialize an empty cell array

% Iterate through each row of allPerTrialData
for row = 1:size(Firing_rates_f, 1)
    for col = 1:size(Firing_rates_f, 2)
        if ~isempty(Firing_rates_f{row, col})
            % Add non-empty cell to allMySpikes
            allMySpikes_Pavlovian{end + 1, 1} = Firing_rates_f{row, col};
        end
    end
end

% Number of cells in allMySpikes_Pavlovian
numCells = size(allMySpikes_Pavlovian, 1);

% Preallocating cells for each trial type
rew30co_spikecounts = cell(numCells, 1);
rew60co_spikecounts = cell(numCells, 1);
rew90co_spikecounts = cell(numCells, 1);
rew120co_spikecounts = cell(numCells, 1);
rew150co_spikecounts = cell(numCells, 1);
rew180co_spikecounts = cell(numCells, 1);
air30co_spikecounts = cell(numCells, 1);
air60co_spikecounts = cell(numCells, 1);
air90co_spikecounts = cell(numCells, 1);
air120co_spikecounts = cell(numCells, 1);
air150co_spikecounts = cell(numCells, 1);
air180co_spikecounts = cell(numCells, 1);
rew30rd_spikecounts = cell(numCells, 1);
rew60rd_spikecounts = cell(numCells, 1);
rew90rd_spikecounts = cell(numCells, 1);
rew120rd_spikecounts = cell(numCells, 1);
rew150rd_spikecounts = cell(numCells, 1);
rew180rd_spikecounts = cell(numCells, 1);
air30rd_spikecounts = cell(numCells, 1);
air60rd_spikecounts = cell(numCells, 1);
air90rd_spikecounts = cell(numCells, 1);
air120rd_spikecounts = cell(numCells, 1);
air150rd_spikecounts = cell(numCells, 1);
air180rd_spikecounts = cell(numCells, 1);

for i = 1:numCells
    data = allMySpikes_Pavlovian{i, 1}; % Get the 44x80 matrix from each cell
    
    % Extracting rows for each trial type. Assumes specific rows correspond to specific trials.
    rew30co_spikecounts{i} = data(1, :);
    rew60co_spikecounts{i} = data(2, :);
    rew90co_spikecounts{i} = data(3, :);
    rew120co_spikecounts{i} = data(4, :);
    rew150co_spikecounts{i} = data(5, :);
    rew180co_spikecounts{i} = data(6, :);
    air30co_spikecounts{i} = data(7, :);
    air60co_spikecounts{i} = data(8, :);
    air90co_spikecounts{i} = data(9, :);
    air120co_spikecounts{i} = data(10, :);
    air150co_spikecounts{i} = data(11, :);
    air180co_spikecounts{i} = data(12, :);
    rew30rd_spikecounts{i} = data(13, :);
    rew60rd_spikecounts{i} = data(14, :);
    rew90rd_spikecounts{i} = data(15, :);
    rew120rd_spikecounts{i} = data(16, :);
    rew150rd_spikecounts{i} = data(17, :);
    rew180rd_spikecounts{i} = data(18, :);
    air30rd_spikecounts{i} = data(19, :);
    air60rd_spikecounts{i} = data(20, :);
    air90rd_spikecounts{i} = data(21, :);
    air120rd_spikecounts{i} = data(22, :);
    air150rd_spikecounts{i} = data(23, :);
    air180rd_spikecounts{i} = data(24, :);
end


Pavlovian_spikes=[rew30co_spikecounts, rew60co_spikecounts,rew90co_spikecounts, rew120co_spikecounts,rew150co_spikecounts, rew180co_spikecounts,...
    air30co_spikecounts, air60co_spikecounts,air90co_spikecounts, air120co_spikecounts,air150co_spikecounts, air180co_spikecounts,...
    rew30rd_spikecounts, rew60rd_spikecounts,rew90rd_spikecounts, rew120rd_spikecounts,rew150rd_spikecounts, rew180rd_spikecounts,...
    air30rd_spikecounts, air60rd_spikecounts,air90rd_spikecounts, air120rd_spikecounts,air150rd_spikecounts, air180rd_spikecounts];



allMySpikes = {}; % Initialize an empty cell array

% Iterate through each row of allPerTrialData
for row = 1:size(allPerTrialData, 1)
    for col = 1:size(allPerTrialData, 2)
        if ~isempty(allPerTrialData{row, col})
            % Add non-empty cell to allMySpikes
            allMySpikes{end + 1, 1} = allPerTrialData{row, col};
        end
    end
end

% Initialize the column vectors as cell arrays
cue_spikecounts = cell(size(allMySpikes, 1), 1);
airpuff_spikecounts = cell(size(allMySpikes, 1), 1);
smallreward_spikecounts = cell(size(allMySpikes, 1), 1);
bigreward_spikecounts = cell(size(allMySpikes, 1), 1);
redcue_spikecounts = cell(size(allMySpikes, 1), 1);
yellowcue_spikecounts = cell(size(allMySpikes, 1), 1);
redreward_spikecounts = cell(size(allMySpikes, 1), 1);
yellowairpuff_spikecounts = cell(size(allMySpikes, 1), 1);

% Loop over each row in allMySpikes
for i = 1:size(allMySpikes, 1)
    % Extract the matrix at position {1, 33} for cue_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 33}) % Check if the cell is not empty
        cue_spikecounts{i, 1} = allMySpikes{i, 1}{1, 33};
    else
        cue_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end
    
    % Extract the matrix at position {1, 39} for airpuff_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 39}) % Check if the cell is not empty
        airpuff_spikecounts{i, 1} = allMySpikes{i, 1}{1, 39};
    else
        airpuff_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

        % Extract the matrix at position {1, 41} for bigreward_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 41}) % Check if the cell is not empty
        bigreward_spikecounts{i, 1} = allMySpikes{i, 1}{1, 41};
    else
        bigreward_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end


    % Extract the matrix at position {1, 44} for smallreward_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 44}) % Check if the cell is not empty
        smallreward_spikecounts{i, 1} = allMySpikes{i, 1}{1, 44};
    else
        smallreward_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

        % Extract the matrix at position {1, 15} for redcue_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 15}) % Check if the cell is not empty
        redcue_spikecounts{i, 1} = allMySpikes{i, 1}{1, 15};
    else
        redcue_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

        % Extract the matrix at position {1, 11} for yellowcue_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 11}) % Check if the cell is not empty
        yellowcue_spikecounts{i, 1} = allMySpikes{i, 1}{1, 11};
    else
        yellowcue_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

        % Extract the matrix at position {1, 17} for redreward_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 17}) % Check if the cell is not empty
        redreward_spikecounts{i, 1} = allMySpikes{i, 1}{1, 17};
    else
        redreward_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

        % Extract the matrix at position {1, 38} for yellowairpuff_spikecounts
    if ~isempty(allMySpikes{i, 1}{1, 38}) % Check if the cell is not empty
        yellowairpuff_spikecounts{i, 1} = allMySpikes{i, 1}{1, 38};
    else
        yellowairpuff_spikecounts{i, 1} = []; % Assign an empty array if the cell is empty
    end

end

% cue_spikecounts and airpuff_spikecounts are now filled with the matrices

% Initialize an empty cell array for the results
sumOfCoolSpikes = {};

% Get the size of the allSpikesPerBin cell array
[nRows, nCols] = size(allSpikesPerBin);

% Initialize a counter for the index in sumOfCoolSpikes
index = 1;

% Loop over each row
for i = 1:nRows
    % Then loop over each column
    for j = 1:nCols
        % Check if the current cell is not empty
        if ~isempty(allSpikesPerBin{i, j})
            % If it's not empty, add its content to sumOfCoolSpikes
            sumOfCoolSpikes{index, 1} = allSpikesPerBin{i, j};
            % Increment the index for sumOfCoolSpikes
            index = index + 1;
        end
    end
end
...
linesOfInterest = [14, 33, 39, 44, 19, 43, 15, 11, 17, 38, 41];
nEvents = length(linesOfInterest);

% Initialize matrices to store the ratios and chi-square statistics for each event
ratio_spikes = zeros(length(sumOfCoolSpikes), nEvents);
chi_square_stats = zeros(length(sumOfCoolSpikes), nEvents);

% Loop through each cell in sumOfCoolSpikes
for i = 1:length(sumOfCoolSpikes)
    % Access the current spike data
    currentSpikes = sumOfCoolSpikes{i};
    
    % Ensure we have enough rows to avoid index out of bounds
    if size(currentSpikes, 1) >= max(linesOfInterest)
        % Process each event
        for j = 1:nEvents
            eventLine = linesOfInterest(j);
            
            % Calculate sums for the specified column ranges
            sum_21_40 = sum(currentSpikes(eventLine, 21:40), 2);
            sum_41_60 = sum(currentSpikes(eventLine, 41:60), 2);
            
            % Calculate and store ratios, avoiding division by zero
            ratio_spikes(i, j) = sum_41_60 / (sum_21_40 + eps);
            
            % Perform chi-square test and store results
            chi_square_stats(i, j) = dg_chi2test2([sum_41_60, sum_21_40]);
        end
    else
        % Handle cases where there are not enough rows
        ratio_spikes(i, :) = NaN;
        chi_square_stats(i, :) = NaN;
    end
end

chi_square_stats_cell=num2cell(chi_square_stats);


%%%%%% stats for everything
% Define the lines of interest
fullSequence = 1:55;
linesOfInterest_all = fullSequence;
nEvents = length(linesOfInterest_all);

% Initialize matrices to store the ratios and chi-square statistics for each event
ratio_spikes_all = zeros(length(sumOfCoolSpikes), nEvents);
chi_square_stats_all = zeros(length(sumOfCoolSpikes), nEvents);
...
for i = 1:length(sumOfCoolSpikes)
    ...
    % Ensure we have enough rows to avoid index out of bounds
    if size(currentSpikes, 1) >= max(linesOfInterest_all)
        % Process each event
        for j = 1:nEvents
            eventLine = linesOfInterest_all(j);
            ...
            % Calculate and store ratios, avoiding division by zero
            ratio_spikes_all(i, j) = sum_41_60 / (sum_21_40 + eps);
            
            % Perform chi-square test and store results
            chi_square_stats_all(i, j) = dg_chi2test2([sum_41_60, sum_21_40]);
        end
    else
        % Handle cases where there are not enough rows
        ratio_spikes_all(i, :) = NaN;
        chi_square_stats_all(i, :) = NaN;
    end
end

chi_square_stats_cell_all=num2cell(chi_square_stats_all);

% ratio_spikes and chi_square_stats are now populated for each event
% Initialize vectors for the ratios
ratio_cue_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_airpuff_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_smallreward_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_bigreward_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_redcue_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_yellowcue_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_redreward_spikes = zeros(length(sumOfCoolSpikes), 1);
ratio_yellowairpuff_spikes = zeros(length(sumOfCoolSpikes), 1);


% Initialize vectors for chi-square statistics
chi_square_cue = zeros(length(sumOfCoolSpikes), 1);
chi_square_airpuff = zeros(length(sumOfCoolSpikes), 1);
chi_square_smallreward = zeros(length(sumOfCoolSpikes), 1);
chi_square_bigreward = zeros(length(sumOfCoolSpikes), 1);
chi_square_redcue = zeros(length(sumOfCoolSpikes), 1);
chi_square_yellowcue = zeros(length(sumOfCoolSpikes), 1);
chi_square_redreward= zeros(length(sumOfCoolSpikes), 1);
chi_square_yellowairpuff = zeros(length(sumOfCoolSpikes), 1);
...
for i = 1:length(sumOfCoolSpikes)
    ...
    % Ensure we have enough rows to avoid index out of bounds
    if size(currentSpikes, 1) >= 39
        % Calculate sums for cue spikes (line 33)
        sum_cue_26_40 = sum(currentSpikes(33, 21:40), 2);
        sum_cue_41_55 = sum(currentSpikes(33, 41:60), 2);

        % Calculate sums for airpuff spikes (line 39)
        sum_airpuff_26_40 = sum(currentSpikes(39, 21:40), 2);
        sum_airpuff_41_55 = sum(currentSpikes(39, 41:60), 2);

        % Calculate sums for small reward spikes (line 44)
        sum_smallreward_26_40 = sum(currentSpikes(44, 21:40), 2);
        sum_smallreward_41_55 = sum(currentSpikes(44, 41:60), 2);

        % Calculate sums for big reward spikes (line 44)
        sum_bigreward_26_40 = sum(currentSpikes(41, 21:40), 2);
        sum_bigreward_41_55 = sum(currentSpikes(41, 41:60), 2);

        % Calculate sums for red cue spikes (line 15)
        sum_redcue_26_40 = sum(currentSpikes(15, 21:40), 2);
        sum_redcue_41_55 = sum(currentSpikes(15, 41:60), 2);

        % Calculate sums for red cue spikes (line 11)
        sum_yellowcue_26_40 = sum(currentSpikes(11, 21:40), 2);
        sum_yellowcue_41_55 = sum(currentSpikes(11, 41:60), 2);

        % Calculate sums for red reward spikes (line 17)
        sum_redreward_26_40 = sum(currentSpikes(17, 21:40), 2);
        sum_redreward_41_55 = sum(currentSpikes(17, 41:60), 2);

        % Calculate sums for yellow airpuff spikes (line 38)
        sum_yellowairpuff_26_40 = sum(currentSpikes(38, 21:40), 2);
        sum_yellowairpuff_41_55 = sum(currentSpikes(38, 41:60), 2);

        % Calculate and store ratios, avoiding division by zero
        ratio_cue_spikes(i) = sum_cue_41_55 / (sum_cue_26_40 + eps);
        ratio_airpuff_spikes(i) = sum_airpuff_41_55 / (sum_airpuff_26_40 + eps);
        ratio_smallreward_spikes(i) = sum_smallreward_41_55 / (sum_smallreward_26_40 + eps);
        ratio_bigreward_spikes(i) = sum_bigreward_41_55 / (sum_bigreward_26_40 + eps);
        ratio_redcue_spikes(i) = sum_redcue_41_55 / (sum_redcue_26_40 + eps);
        ratio_yellowcue_spikes(i) = sum_yellowcue_41_55 / (sum_yellowcue_26_40 + eps);
        ratio_redreward_spikes(i) = sum_redreward_41_55 / (sum_redreward_26_40 + eps);
        ratio_yellowairpuff_spikes(i) = sum_yellowairpuff_41_55 / (sum_yellowairpuff_26_40 + eps);


        % Perform chi-square test with reversed order and store results
        chi_square_cue(i) = dg_chi2test2([sum_cue_41_55, sum_cue_26_40]);
        chi_square_airpuff(i) = dg_chi2test2([sum_airpuff_41_55, sum_airpuff_26_40]);
        chi_square_smallreward(i) = dg_chi2test2([sum_smallreward_41_55, sum_smallreward_26_40]);
        chi_square_bigreward(i) = dg_chi2test2([sum_bigreward_41_55, sum_bigreward_26_40]);
        chi_square_redcue(i) = dg_chi2test2([sum_redcue_41_55, sum_redcue_26_40]);
        chi_square_yellowcue(i) = dg_chi2test2([sum_yellowcue_41_55, sum_yellowcue_26_40]);
        chi_square_redreward(i) = dg_chi2test2([sum_redreward_41_55, sum_redreward_26_40]);
        chi_square_yellowairpuff(i) = dg_chi2test2([sum_yellowairpuff_41_55, sum_yellowairpuff_26_40]);



    else
        % Handle cases where there are not enough rows
        ratio_cue_spikes(i) = NaN;
        ratio_airpuff_spikes(i) = NaN;
        ratio_smallreward_spikes(i) = NaN;
        ratio_bigreward_spikes(i) = NaN;
        ratio_redcue_spikes(i) = NaN;
        ratio_yellowcue_spikes(i) = NaN;
        ratio_redreward_spikes(i) = NaN;
        ratio_yellowairpuff_spikes(i) = NaN;

        chi_square_cue(i) = NaN;
        chi_square_airpuff(i) = NaN;
        chi_square_smallreward(i) = NaN;
        chi_square_bigreward(i) = NaN;
        chi_square_redcue(i) = NaN;
        chi_square_yellowcue(i) = NaN;
        chi_square_redreward(i) = NaN;
        chi_square_yellowairpuff(i) = NaN;

    end
end

% Initialize the sign columns with the same length as the ratio columns
ratio_cue_sign = zeros(length(ratio_cue_spikes), 1);
ratio_airpuff_sign = zeros(length(ratio_airpuff_spikes), 1);
ratio_smallreward_sign = zeros(length(ratio_smallreward_spikes), 1);
ratio_bigreward_sign = zeros(length(ratio_bigreward_spikes), 1);
ratio_redcue_sign = zeros(length(ratio_redcue_spikes), 1);
ratio_yellowcue_sign = zeros(length(ratio_yellowcue_spikes), 1);
ratio_redreward_sign = zeros(length(ratio_redreward_spikes), 1);
ratio_yellowairpuff_sign = zeros(length(ratio_yellowairpuff_spikes), 1);


% Loop through the ratio_cue_spikes to determine the sign
for i = 1:length(ratio_cue_spikes)
    if ratio_cue_spikes(i) > 1
        ratio_cue_sign(i) = 1;
    elseif ratio_cue_spikes(i) < 1
        ratio_cue_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end

% Loop through the ratio_airpuff_spikes to determine the sign
for i = 1:length(ratio_airpuff_spikes)
    if ratio_airpuff_spikes(i) > 1
        ratio_airpuff_sign(i) = 1;
    elseif ratio_airpuff_spikes(i) < 1
        ratio_airpuff_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end

% Loop through the ratio_smallreward_spikes to determine the sign
for i = 1:length(ratio_smallreward_spikes)
    if ratio_smallreward_spikes(i) > 1
        ratio_smallreward_sign(i) = 1;
    elseif ratio_smallreward_spikes(i) < 1
        ratio_smallreward_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end
...
% Loop through the ratio_smallreward_spikes to determine the sign
for i = 1:length(ratio_bigreward_spikes)
    if ratio_bigreward_spikes(i) > 1
        ratio_bigreward_sign(i) = 1;
    elseif ratio_bigreward_spikes(i) < 1
        ratio_bigreward_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end

% Loop through the ratio_redcue_spikes to determine the sign
for i = 1:length(ratio_redcue_spikes)
    if ratio_redcue_spikes(i) > 1
        ratio_redcue_sign(i) = 1;
    elseif ratio_redcue_spikes(i) < 1
        ratio_redcue_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end
...
% Loop through the ratio_smallreward_spikes to determine the sign
for i = 1:length(ratio_yellowcue_spikes)
    if ratio_yellowcue_spikes(i) > 1
        ratio_yellowcue_sign(i) = 1;
    elseif ratio_yellowcue_spikes(i) < 1
        ratio_yellowcue_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end
...
% Loop through the ratio_smallreward_spikes to determine the sign
for i = 1:length(ratio_redreward_spikes)
    if ratio_redreward_spikes(i) > 1
        ratio_redreward_sign(i) = 1;
    elseif ratio_redreward_spikes(i) < 1
        ratio_redreward_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end
...
% Loop through the ratio_smallreward_spikes to determine the sign
for i = 1:length(ratio_yellowairpuff_spikes)
    if ratio_yellowairpuff_spikes(i) > 1
        ratio_yellowairpuff_sign(i) = 1;
    elseif ratio_yellowairpuff_spikes(i) < 1
        ratio_yellowairpuff_sign(i) = -1;
    % No need to explicitly handle == 0 case, as it's already set to 0 by initialization
    end
end

% Convert ratio_cue_sign and ratio_airpuff_sign and ratio_smallreward_sign to cell arrays
ratio_cue_sign_cell = num2cell(ratio_cue_sign);
ratio_airpuff_sign_cell = num2cell(ratio_airpuff_sign);
ratio_smallreward_sign_cell = num2cell(ratio_smallreward_sign);
ratio_bigreward_sign_cell = num2cell(ratio_bigreward_sign);
ratio_redcue_sign_cell = num2cell(ratio_redcue_sign);
ratio_yellowcue_sign_cell = num2cell(ratio_yellowcue_sign);
ratio_redreward_sign_cell = num2cell(ratio_redreward_sign);
ratio_yellowairpuff_sign_cell = num2cell(ratio_yellowairpuff_sign);

chi_square_cue_cell=num2cell(chi_square_cue);
chi_square_airpuff_cell=num2cell(chi_square_airpuff);
chi_square_smallreward_cell=num2cell(chi_square_smallreward);
chi_square_bigreward_cell=num2cell(chi_square_bigreward);
chi_square_redcue_cell=num2cell(chi_square_redcue);
chi_square_yellowcue_cell=num2cell(chi_square_yellowcue);
chi_square_redreward_cell=num2cell(chi_square_redreward);
chi_square_yellowairpuff_cell=num2cell(chi_square_yellowairpuff);

% Define bin duration in seconds
binDuration = 50 / 1000; % 50 ms converted to seconds

% Initialize cell arrays to store firing rates for the cue, approach, and avoidance periods
firingRatesForCuePeriod = cell(size(allMySpikes, 1), 1);
firingRatesForApproachPeriod = cell(size(allMySpikes, 1), 1);
firingRatesForAvoidancePeriod = cell(size(allMySpikes, 1), 1);
firingRatesForApproachBigRewardPeriod = cell(size(allMySpikes, 1), 1);
firingRatesForRedCuePeriod = cell(size(allMySpikes, 1), 1);
firingRatesForYellowCuePeriod = cell(size(allMySpikes, 1), 1);
firingRatesForRedRewardPeriod = cell(size(allMySpikes, 1), 1);
firingRatesForYellowAirpuffPeriod = cell(size(allMySpikes, 1), 1);

% Iterate over allMySpikes to process the data for each period
for i = 1:size(allMySpikes, 1)
    % For the Cue Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 33
        spikeCountMatrixCue = allMySpikes{i, 1}{1, 33}; % Extract the matrix for the Cue Period
        firingRatesForCuePeriod{i} = spikeCountMatrixCue / binDuration; % Calculate firing rates
    end
    
    % For the Approach Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 41
        spikeCountMatrixApproachBigReward = allMySpikes{i, 1}{1, 41}; % Extract the matrix for the Approach Period
        firingRatesForApproachBigRewardPeriod{i} = spikeCountMatrixApproachBigReward / binDuration; % Calculate firing rates
    end

        % For the Approach Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 39
        spikeCountMatrixApproach = allMySpikes{i, 1}{1, 39}; % Extract the matrix for the Approach Period
        firingRatesForApproachPeriod{i} = spikeCountMatrixApproach / binDuration; % Calculate firing rates
    end

    
    % For the Avoidance Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 44
        spikeCountMatrixAvoidance = allMySpikes{i, 1}{1, 44}; % Extract the matrix for the Avoidance Period
        firingRatesForAvoidancePeriod{i} = spikeCountMatrixAvoidance / binDuration; % Calculate firing rates
    end

        % For the Reward Cue Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 15
        spikeCountMatrixRedCue = allMySpikes{i, 1}{1, 15}; % Extract the matrix for the RedCue Period
        firingRatesForRedCuePeriod{i} = spikeCountMatrixRedCue / binDuration; % Calculate firing rates
    end

        % For the YellowCue Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 11
        spikeCountMatrixYellowCue = allMySpikes{i, 1}{1, 11}; % Extract the matrix for the YellowCue Period
        firingRatesForYellowCuePeriod{i} = spikeCountMatrixYellowCue / binDuration; % Calculate firing rates
    end

        % For the RedReward Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 17
        spikeCountMatrixRedReward = allMySpikes{i, 1}{1, 17}; % Extract the matrix for the RedReward Period
        firingRatesForRedRewardPeriod{i} = spikeCountMatrixRedReward / binDuration; % Calculate firing rates
    end

        % For the YellowAirpuff Period
    if ~isempty(allMySpikes{i, 1}) && length(allMySpikes{i, 1}) >= 38
        spikeCountMatrixYellowAirpuff = allMySpikes{i, 1}{1, 38}; % Extract the matrix for the YellowAirpuff Period
        firingRatesForYellowAirpuffPeriod{i} = spikeCountMatrixYellowAirpuff / binDuration; % Calculate firing rates
    end
end

% Initialize cell arrays to store the average firing rates for each matrix for each period
averagedFiringRates_cue = cell(length(firingRatesForCuePeriod), 1);
averagedFiringRates_Approach = cell(length(firingRatesForApproachPeriod), 1);
averagedFiringRates_ApproachBigReward = cell(length(firingRatesForApproachBigRewardPeriod), 1);
averagedFiringRates_Avoidance = cell(length(firingRatesForAvoidancePeriod), 1);
averagedFiringRates_RedCue = cell(length(firingRatesForRedCuePeriod), 1);
averagedFiringRates_YellowCue = cell(length(firingRatesForYellowCuePeriod), 1);
averagedFiringRates_RedReward = cell(length(firingRatesForRedRewardPeriod), 1);
averagedFiringRates_YellowAirpuff = cell(length(firingRatesForYellowAirpuffPeriod), 1);


% Calculate and store the average firing rate for each matrix for each period
for i = 1:length(firingRatesForCuePeriod)
    % For the Cue Period
    if ~isempty(firingRatesForCuePeriod{i})
        averagedFiringRates_cue{i} = mean(firingRatesForCuePeriod{i}, 1);
    end
    
    % For the Approach Period
    if ~isempty(firingRatesForApproachPeriod{i})
        averagedFiringRates_Approach{i} = mean(firingRatesForApproachPeriod{i}, 1);
    end
    
        % For the Approach BigRewardPeriod
    if ~isempty(firingRatesForApproachBigRewardPeriod{i})
        averagedFiringRates_ApproachBigReward{i} = mean(firingRatesForApproachBigRewardPeriod{i}, 1);
    end


    % For the Avoidance Period
    if ~isempty(firingRatesForAvoidancePeriod{i})
        averagedFiringRates_Avoidance{i} = mean(firingRatesForAvoidancePeriod{i}, 1);
    end

        % For the RedCue Period
    if ~isempty(firingRatesForRedCuePeriod{i})
        averagedFiringRates_RedCue{i} = mean(firingRatesForRedCuePeriod{i}, 1);
    end

        % For the YellowCue Period
    if ~isempty(firingRatesForYellowCuePeriod{i})
        averagedFiringRates_YellowCue{i} = mean(firingRatesForYellowCuePeriod{i}, 1);
    end

        % For the RedReward Period
    if ~isempty(firingRatesForRedRewardPeriod{i})
        averagedFiringRates_RedReward{i} = mean(firingRatesForRedRewardPeriod{i}, 1);
    end

        % For the YellowAirpuff Period
    if ~isempty(firingRatesForYellowAirpuffPeriod{i})
        averagedFiringRates_YellowAirpuff{i} = mean(firingRatesForYellowAirpuffPeriod{i}, 1);
    end
end

% Copy ratio_spikes to a new variable ratio_spikes_sign
ratio_spikes_sign = ratio_spikes;

% Values less than 1 (but not zero) are set to -1 in ratio_spikes_sign
ratio_spikes_sign(ratio_spikes_sign < 1 & ratio_spikes_sign ~= 0) = -1;

% Values greater than 1 are set to 1 in ratio_spikes_sign
ratio_spikes_sign(ratio_spikes_sign > 1) = 1;

% Values exactly equal to 0 remain unchanged, so no action is needed for them.

ratio_spikes_sign_cell=num2cell(ratio_spikes_sign);

% Assume mdl is the original 2553x4 cell array and allPerTrialData is also 2553x4
mdl_step1 = results;  % Copy mdl to mdl_step1

% Dimensions of the cell arrays
[numRows, numCols] = size(mdl_step1);  % Both arrays have the same dimensions

% Loop through each cell
for row = 1:numRows
    for col = 1:numCols
        % Check if the cell in mdl is empty and the corresponding cell in allPerTrialData is not empty
        if isempty(mdl_step1{row, col}) && ~isempty(allPerTrialData{row, col})
            mdl_step1{row, col} = 'Error';
        end
    end
end

% Initialize an empty cell array to hold the non-empty values
nonEmptyCells = {};

% Iterate through each row and then each column of mdl_step1
for row = 1:size(mdl_step1, 1)
    for col = 1:size(mdl_step1, 2)
        if ~isempty(mdl_step1{row, col})
            nonEmptyCells{end+1, 1} = mdl_step1{row, col};  % Note the ",1" to ensure column vector
        end
    end
end

% Convert the cell array to a column vector
% If the contents are numeric and consistent, you can concatenate them
% For mixed or non-numeric data, keep as a cell array
if all(cellfun(@isnumeric, nonEmptyCells))
    columnVector = vertcat(nonEmptyCells{:});
else
    columnVector = nonEmptyCells;  % Keep as cell array for mixed or non-numeric data
end

% Assuming 'columnVector' is already filled with structs or 'Error' strings
numElements = length(columnVector);

% Cell arrays to hold the processed data
inModelIndex = cell(numElements, 1);
correlationValues = cell(numElements, 1);

% Process each element in columnVector
for idx = 1:numElements
    currentItem = columnVector{idx};
    if isstruct(currentItem)  % Check if the item is a struct
        inModel = currentItem.InModel;
        if sum(inModel) == 1 % Check if exactly one true value exists
            trueIndex = find(inModel);  % Get the index of the true value
            % Store the index and correlation coefficient in cells
            inModelIndex{idx} = trueIndex;
            correlationValues{idx} = currentItem.CorrelationCoefficients(trueIndex);
        else
            % If condition is not met (no true or more than one true), append NaN
            inModelIndex{idx} = NaN;
            correlationValues{idx} = NaN;
        end
    elseif strcmp(currentItem, 'Error')
        % Handle 'Error' by appending a distinct identifier, e.g., NaN or a specific value
        inModelIndex{idx} = NaN;  % Or another value to mark 'Error'
        correlationValues{idx} = NaN;  % Or another value to mark 'Error'
    else
        % Assume it is another non-struct case, handle similarly to 'Error'
        inModelIndex{idx} = NaN;
        correlationValues{idx} = NaN;
    end
end

% Initialize an empty cell array for the predictors
columnVector_predictors = {};

% Iterate through each cell in columnVector
for i = 1:length(columnVector)
    % Check if the cell does not contain 'Error'
    if ~strcmp(columnVector{i, 1}, 'Error')
        % Access the PredictorNames and store them
        columnVector_predictors{end+1, 1} = columnVector{i, 1}.InModel;
    else
        % If it's 'Error', add 'Error' to the new vector and move on
        columnVector_predictors{end+1, 1} = 'Error';
    end
end

% Assuming columnVector_predictors is the initial 2095x1 cell array
columnVector_predictors_x = cell(size(columnVector_predictors, 1), 1);

for i = 1:size(columnVector_predictors, 1)
    currentCell = columnVector_predictors{i, 1}; % Get the current cell's content
    trueIndices = find(currentCell == 1); % Find indices where values are exactly 1
    
    % Prepare a new cell array for these indices
    newXCell = cell(length(trueIndices), 1);
    for j = 1:length(trueIndices)
        newXCell{j, 1} = ['x', num2str(trueIndices(j))];
    end
    
    % Assign this new cell array to the corresponding position in the new vector
    columnVector_predictors_x{i, 1} = newXCell;
end

% Assume monkey_name is a string array with elements like "\"CSC28_in1\""

% Step 1: Remove double quotes from each string element
% This creates a new string array where the double quotes have been stripped.
monkey_name_no_quotes = strip(monkey_name, '"');

% Step 2: Convert the modified string array to a cell array
monkey_name_cell = cellstr(monkey_name_no_quotes);


fromStW1=[dates,fileNames,counts1,monkey_name,allInfoTogether,columnVector_predictors_x,averagedFiringRates_cue,ratio_cue_sign_cell,ratio_airpuff_sign_cell,...
    chi_square_stats_cell,ratio_spikes_sign_cell,averagedFiringRates_Approach,averagedFiringRates_Avoidance,averagedFiringRates_ApproachBigReward,...
    averagedFiringRates_RedCue,averagedFiringRates_YellowCue,averagedFiringRates_RedReward,averagedFiringRates_YellowAirpuff,Pavlovian_spikes,inModelIndex,correlationValues];
% Assuming fromStW1 is a cell array and column 67 contains numeric data

matrix1=fromStW1;
matrix2=fromExcel1;

% Assuming matrix2 is a 1017x6 cell array and the first four columns may contain strings

% % Loop over the first four columns of matrix2
% for col = 1:4
%     for row = 1:size(matrix2, 1)
%         % Check if the current cell contains a string
%         if ischar(matrix2{row, col})
%             % Add single quotes around the string
%             matrix2{row, col} = ['''' matrix2{row, col} ''''];
%         end
%     end
% end

% Create a logical matrix to mark the matching rows
% Assume matrix1 is 1261x94 and matrix2 is 1017x6
% Initialize the extended matrix2 to include additional columns for the matching data
extendedMatrix2 = [matrix2, cell(size(matrix2, 1), size(matrix1, 2) - size(matrix2, 2))];

% Prepare keys for comparison by concatenating the first four columns
% This assumes the columns are strings or can be converted to strings directly
keyMatrix1 = strcat(matrix1(:,1), matrix1(:,2), matrix1(:,3), matrix1(:,4));
keyMatrix2 = strcat(matrix2(:,1), matrix2(:,2), matrix2(:,3), matrix2(:,4));

% Iterate through each row in matrix2 to find matches in matrix1
for i = 1:size(matrix2, 1)
    % Find the index in matrix1 where the keys match
    matchIndex = find(strcmp(keyMatrix1, keyMatrix2{i}), 1, 'first');

    % If a match is found, append the data from matrix1 to matrix2
    if ~isempty(matchIndex)
        % Append the necessary columns from matrix1 to the current row in extendedMatrix2
        % Here we append columns 7 to 94 from matrix1
        extendedMatrix2(i, 7:size(matrix1, 2)) = matrix1(matchIndex, 7:end);
    end
end

tomanipulate=extendedMatrix2;

% Now filtered_manipulation contains only the rows that do not have 'MUA', 'poorunit' in the 5th column, or 'Error' in the 62nd column
% Initialize a new cell array to hold the filtered data
filtered_manipulation = {};

% the original matrix
extendedMatrix2; % 1017x118 cell matrix

% Find rows with empty cells in column 7
emptyRows = cellfun(@isempty, extendedMatrix2(:, 7));

% Remove these rows
extendedMatrix2_clean = extendedMatrix2(~emptyRows, :);

% Assuming klasidia is the initial 973x103 cell matrix
klasidia=extendedMatrix2;
% Step 1: Filter to keep rows with "pACC" or "cOFC" in the 6th column
rowsToKeep = strcmp(klasidia(:,6), 'pACC') | strcmp(klasidia(:,6), 'cOFC');
klasidiaFiltered = klasidia(rowsToKeep, :);

% Step 2: Further filter to keep rows with "goodunit" or "OKunit" in the 5th column of the already filtered data
rowsToKeepFurther = strcmp(klasidiaFiltered(:,5), 'goodunit') | strcmp(klasidiaFiltered(:,5), 'OKunit');
klasidiaFilteredFinal = klasidiaFiltered(rowsToKeepFurther, :);

% Step 3: Count occurrences of "cOFC" and "pACC" in the 6th column of the final filtered data
count_cOFC_final = sum(strcmp(klasidiaFilteredFinal(:,6), 'cOFC'));
count_pACC_final = sum(strcmp(klasidiaFilteredFinal(:,6), 'pACC'));

% Display the counts
disp(['Number of "cOFC" occurrences in the final filtered data: ', num2str(count_cOFC_final)]);
disp(['Number of "pACC" occurrences in the final filtered data: ', num2str(count_pACC_final)]);

filtered_manipulation=klasidiaFilteredFinal;
...
for i = 1:size(filtered_manipulation, 1)
    ...
    rowData = cell2mat(filtered_manipulation(i, 66:79));

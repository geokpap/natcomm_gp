...
%$Rev: 96 $
%$Date: 2009-11-12 15:47:18 -0500 (Thu, 12 Nov 2009) $
%$Author: dgibson $
...
for fileidx = 1:length(filenums)
    ...
    for trial = trials_list
        ...
        if avgflag || popflag || EPflag
            ...
            %Plot event markers as in lfp_disp
            hold on;
            hA = gca;
    ...
    if avgflag || popflag || EPflag
        ...
        if ~(isequal(windowing, 'coh') && popflag)
            ...
            %Plot event marker(s) as in lfp_disp
            hA = gca;
            hold on;
            eventtime = 0;
            eventcolor = '';
            if lfp_AlignmentRef(1) <= length(lfp_EventColors)
                eventcolor = lfp_EventColors{lfp_AlignmentRef(1)};
            end
            if isempty(eventcolor)
                eventcolor = lfp_EventDefaultColor;
            end
            ...
            set(hL, ...
                'ButtonDownFcn', ...
                ['fprintf(1,''Alignment Reference, t=0\n'')'] );
            if ~isempty(evts2avg)
                ...
                for ID = evtIDs
                    ...
                    detailstr = sprintf('ID=%d, "%s", color=%s approx stats\\nmedian=%6.4g, mean=%6.4g, SD=%6.4g\\nfirst quartile=%6.4g, last quartile=%6.4g\\n5th percentile=%6.4g, 95th percentile=%6.4g', ...
                        ID, eventname, colorstr, medianTS, mean(TS), std(TS), prct(3), prct(4), prct(1), prct(2));
...
if isequal(windowing, 'coh') && (popflag)
    ...
    [thresh, cohdisp] = lfp_coherogram3(label, cohdata1, cohdata2, ...
        moving_win, 1, padfactor, absflag, arrowflag, ...
        minspikes, NW, K, rmdcflag, rotateflag, p );
    if shuffleflag || printsigflag || logsigflag
        % compute p2, the probability of getting the
        % observed number or larger of "significant" cells

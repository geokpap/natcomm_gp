...
% one lfp_lib-format '.evtsav' with the same base filename as the input; a
% photometry file produces one dg_Nlx2Mat-format '.mat' CSC format output
% file for each channel, named by concatenating the column header onto the
% base filename of <pathname>.
%OPTIONS
...
switch numcols
    case 3
        ...
        % Trial parameters must have the exact same timestamp as the event
        % 7 ('TTL_EVT.TRIAL_START') that identifies the trial.
        evt7idx = find(allTTL == 7);
        numtrials = length(evt7idx);
        lfp_save_params = cell(numtrials, 1);
        for trialidx = 1:numtrials
            trialTS = allTS(evt7idx(trialidx));
            trialnum = allval(evt7idx(trialidx));
            TPidx = find(allTS == trialTS);
            if length(TPidx) ~= 7
                warning('dg_AM2Mat:TPidx', ...
                    'Wrong number of params for trial # %d (index %d).', ...
                    trialnum, trialidx);
            else
    ...
    otherwise
        ...
        for chnum = 1:numch
            ...
            dg_Nlx2Mat_Timestamps = round(reshape( ...
                str2double(cells(2:end, 1)), 1, [] ) * 1e3); 

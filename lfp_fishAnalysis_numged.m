...
%TODO:
%   write an SUA section of the analysis (maybe adapting the MUA section)
...
%       Applies both to LFP and spike analyses.
...
%$Rev: 279 $
%$Date: 2012-07-16 17:43:39 -0400 (Mon, 16 Jul 2012) $
%$Author: dgibson $
...
for colnum = 1:ncols
    ...
    % directoryand blocknum might be different from colnum (e.g. if there
    % is a block missing):
    ...
    if blockdirs(colnum).isdir && blocknum > 0
        ...
        if isempty(vmaxidx) && isempty(vminidx)
            ...
            %   This is where the infinite horror of noisy baselines rears
            % its ugly head and requires us to calculate new CLs when
            % blspec is not 'block'.
            SEM = EPdata(extrmidx(1:end-1),3,colnum)/2;
            if ~isempty(blevtIDs)
                lfp_AlignmentRef = blevtIDs;
                [junk, data] = lfp_disp([], lfp_ActiveFilenums(1), ...
                    blwin, 'ovr', 'noplot'); %#ok<ASGLU>
                blvals = data(:);
                blSEM2 = var(blvals) / numel(blvals);
                SEM = sqrt(blSEM2 * ones(size(SEM)) + SEM .* SEM);
            end
            ...
            % Throw away any that are not significantly
            % different from baseline, which we here define as the ones
            % whose CLs include <meanbaseline>:
            badones = abs(EPdata(extrmidx(sigidx2),2,colnum)) ...
                <= opts.numSEMs*EPdata(extrmidx(sigidx2),3,colnum)/2;
            sigidx2(badones) = [];

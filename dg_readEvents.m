function [TS, TTL, ES, Hdr, nlxjunk] = dg_readEvents(filename, varargin)
...
%   TTL: row vector of TTL IDs.  These are returned as signed integers,
%       with 2^15 bit (strobe/sync) propagated to the left.  To extract the
%       lower 15 bits, add 2^15 to negative values to yield positive
%       integers.
%   ES:  column cell vector of Event strings.
%   nlxjunk: all the other CSC fields that for our usual purposes are junk,
%       named as in the Nlx2MatEV_411 header comments (or see code here).
% OPTIONS
...
try
    if ispc
        ...
        if nargout > 4
            [TS, nlxjunk.EventIDs, TTL, nlxjunk.Extras, ES, Hdr] = ...
                Nlx2MatEV_411(file2read, ...
                [1, 1, 1, 1, 1], 1, modenum, modearg);
        else
            [TS, TTL, ES, Hdr] = Nlx2MatEV_411(file2read, ...
                [1, 0, 1, 0, 1], 1, modenum, modearg);
        end
        if ~unsignedflag

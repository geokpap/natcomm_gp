function hh = dg_polarbar(x, y, l,u,symbol)
%dg_polarbar Polar error bar plot.
%   Not meant to be reminiscent of "D. Boon kilt a bar on this tree".
%   dg_polarbar(X,Y,L,U) plots the graph of vector X vs. vector Y with
...
%
%   dg_polarbar(X,Y,E) or dg_polarbar(Y,E) plots Y with error bars [Y-E Y+E].
%   dg_polarbar(...,'LineSpec') uses the color and linestyle specified by
%   the string 'LineSpec'.  See PLOT for possibilities.
%
%   H = dg_polarbar(...) returns a vector of line handles.
%
%   For example,
%      x = 1:10;
%      y = sin(x);
...
%   Dan Gibson copied from Matlab function "errorbar" and simply replaced
%   the calls to "plot" with calls to "polar", and changed the name.
...
h = polar(xb,yb,esymbol); hold on
h = [h;polar(x,y,symbol)]; 

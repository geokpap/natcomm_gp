...
%     'OKunit'
%     'goodunit'
%   ...plus any of the above prepended by:
%       'noK' to indicate that there weren't enough spikes to calculate <K>
%       'trunc' to indicate that the cluster was probably truncated by the
%           triggering threshold (except there is no 'noKMUA', but there is
%           'noKbad')
%       'inspect-' to indicate that the cluster may be poorly cut
%       'poss-' to indicate that the cluster may be a a good unit that was
%           misclassified by the numerical criteria
%   ...plus 'noKOKiso', 'noKgoodiso' for clusters with 'OK' or 'good'
%   isolation measurements but 'noK' value.
%NOTES
...
%$Rev: 302 $
%$Date: 2022-10-17 19:22:58 -0400 (Mon, 17 Oct 2022) $
%$Author: dgibson $
...
if isnan(LRatio)
    ...
    elseif isequal(cat, 'goodiso')
        % good isolation
        if (K < 1.6) && (SNR > 3.3)
            if trunc==0 && rspkwav < 0.5
                % neither truncated nor overly greedy
                cat = 'goodunit';
            elseif (rspkwav >= 0.05 && rspkwav < 0.2 && LRatio < 0.03) ...
                    || (rspkwav < 0.05 && ratioratio < 1.5)
                % truncated, but exceptionally good isolation
                cat = 'goodunit';
            else
                % 'inspect-goodiso':
                inspectflag = true;
...
if ismember(cat, {'OKunit' 'OKiso'})
    if ratioratio > 2.2 && SNR > 3.3
        % With that good an SNR, and that bad a <ratioratio>, it's probably
        % mis-cut.
        inspectflag = true;
    elseif ~ismember(cat, {'goodunit' 'goodiso'})
...
%   'MUA'
ratioratio = LRatio/LRatioThresh;
goodiso = (rspkwav < 0.05 && ratioratio < 2.5) || ...
